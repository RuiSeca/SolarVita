...
class FitnessTrackerWidget extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<FitnessTrackerWidget> createState() => _FitnessTrackerWidgetState();
}
...
class _FitnessTrackerWidgetState extends ConsumerState<FitnessTrackerWidget>
  ...
  Widget build(BuildContext context) {
    ...
    final carbonSavings = 4.2; // Mock value since carbon provider needs to be implemented
      ...
      child: ClipRRect(
        ...
        child: Stack(
          children: [
            ...
            _buildFitnessStatsOverlay(context, steps, activeMinutes, carbonSavings),
  ...
  Widget _buildFitnessStatsOverlay(BuildContext context, int steps, int activeMinutes, double carbonSavings) {
    ...
    return Positioned(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _buildQuickStat(context, '$steps', 'Steps', Icons.directions_walk, Colors.blue[600]!),
              const SizedBox(width: 16),
              _buildQuickStat(context, '${walkingKm.toStringAsFixed(1)}km', 'Distance', Icons.straighten, Colors.green[600]!),
              const SizedBox(width: 16),
              _buildQuickStat(context, '${caloriesBurned}cal', 'Burned', Icons.local_fire_department, Colors.orange[600]!),
            ],
          ),
        ],
      ),
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          return Container(
            ...
            decoration: BoxDecoration(
              color: Colors.green.withValues(alpha: 0.3 - (_pulseController.value * 0.2)),
...
class _RouteVisualizationPainter extends CustomPainter {
    ...
    if (points.isNotEmpty) {
      ...
      for (int i = 1; i < points.length; i++) {
        ...
        if (progress <= routeProgress) {
          ...
          final segmentProgress = (routeProgress - (i - 1) / (points.length - 1)) * (points.length - 1);
          
          final partialX = prevPoint.dx + (currentPoint.dx - prevPoint.dx) * segmentProgress;
          final partialY = prevPoint.dy + (currentPoint.dy - prevPoint.dy) * segmentProgress;
  ...
  bool shouldRepaint(covariant _RouteVisualizationPainter oldDelegate) {
    return oldDelegate.routeProgress != routeProgress || oldDelegate.isDark != isDark;
  }
}