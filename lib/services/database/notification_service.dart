...
class NotificationService {
  ...
  static const String _enhancedNotificationPreferencesKey =
    ...
    try {
      ...
      for (final dayEntry in availableDays.entries) {
        ...
        await _scheduleNotification(
          ...
          body:
              'Time for your ${workoutTypes.isNotEmpty ? workoutTypes.first : "workout"} session!',
          scheduledDate: scheduledTime,
          payload: jsonEncode({
            'type': 'workout',
            'day': day,
            'workoutType': workoutTypes.isNotEmpty
                ? workoutTypes.first
                : 'general',
          }),
        );
  ...
  Future<void> schedulePersonalizedMealReminders({
    ...
    Map<String, String>?
    customMealNames, // mealType -> custom name from meal plan
  }) async {
    if (!settings.enabled) return;
    ...
    try {
      ...
      for (final mealEntry in mealTimes.entries) {
        ...
        if (config.timingType == NotificationTimingType.specificTime &&
          ...
          final randomMinutes =
              (DateTime.now().millisecond % 30) - 15; // ±15 minutes
          scheduledTime = _getNextOccurrenceOfTime(
            'today',
            mealTime,
          ).add(Duration(minutes: randomMinutes));
        }

        // Subtract advance minutes
        scheduledTime = scheduledTime.subtract(
          Duration(minutes: config.advanceMinutes),
        );

        // Get meal name (custom from meal plan or default)
        String mealName =
            config.customMealName ??
    ...
    try {
      ...
      for (final notification in pendingNotifications) {
        if (notification.payload != null &&
            notification.payload!.contains('"type":"$type"')) {
  ...
  Future<void> testMealNotificationNow({int offsetMinutes = 1}) async {
    try {
      ...
      await _scheduleNotification(
        ...
        body:
            'This is a test meal notification scheduled for ${testTime.toString().substring(11, 16)}',
  ...
  DateTime _getRandomTimeInPeriod(String day, String period) {
    ...
    return _getNextOccurrenceOfTime(
      day,
      TimeOfDay(hour: randomHour, minute: randomMinute),
    );
  }
