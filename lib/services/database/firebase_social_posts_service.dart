...
class FirebaseSocialPostsService {
  static final FirebaseSocialPostsService _instance =
  ...
  Stream<List<SocialPost>> getPostsFeed({
    ...
    if (pillars != null && pillars.isNotEmpty) {
      query = query.where(
        'pillars',
        arrayContainsAny: pillars.map((p) => p.toString()).toList(),
      );
    }
  ...
  Future<SocialPost> updatePost({
    ...
    try {
      ...
      if (newMediaFiles != null && newMediaFiles.isNotEmpty) {
        final newUrls = await _uploadMediaFiles(
          postId,
          newMediaFiles,
          'images',
        );
  ...
  Future<void> toggleSavePost(String postId) async {
    ...
    if (doc.exists) {
      ...
      // Save the post
      await savedPostsRef.set({'postId': postId, 'savedAt': Timestamp.now()});
    }
  }
  ...
  Future<List<String>> _uploadMediaFiles(
    ...
    for (int i = 0; i < files.length; i++) {
      final file = files[i];
      final fileName =
          '${_uuid.v4()}_${i}_${DateTime.now().millisecondsSinceEpoch}';
      final extension = file.path.split('.').last;
      final ref = _storage.ref().child(
        'posts/$postId/$folder/$fileName.$extension',
      );
  ...
  Future<bool> _canViewPostAsync(
    SocialPost post, {
    PostVisibility? filterVisibility,
  }) async {
    ...
    if (filterVisibility == PostVisibility.supporters) {
      try {
        ...
        if (isSupporting) {
          ...
          return post.visibility == PostVisibility.public ||
              post.visibility == PostVisibility.supporters;
        }
        return false;
      } catch (e) {
        return false;
  ...
  Future<void> _createPostRevision(
    SocialPost oldPost,
    SocialPost newPost,
    String? editReason,
  ) async {
