...
class RoutineService {
  ...
  Future<RoutineManager> reorderExercises(
    ...
    if (oldIndex >= 0 &&
        oldIndex < updatedExercises.length &&
        newIndex >= 0 &&
        newIndex < updatedExercises.length) {
  ...
  Map<String, dynamic> getRoutineStats(WorkoutRoutine routine) {
    ...
    return {
      ...
      'restDays': restDays,
      'averageWorkoutTime': workoutDays > 0
          ? (totalMinutes / workoutDays).round()
          : 0,
  ...
  Future<RoutineManager> duplicateRoutine(
    String routineId,
    String newName,
  ) async {
    final manager = await loadRoutineManager();
    ...
    final originalRoutine = manager.routines.firstWhere(
      (r) => r.id == routineId,
    );
