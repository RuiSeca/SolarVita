...
class FirebaseChatService {
          ...
          for (final doc in snapshot.docs) {
            try {
              ...
              final enrichedConversation =
                  await _enrichConversationWithParticipantData(conversation);
        ...
        try {
          ...
          if (currentUser != null && participantId == currentUser.uid) {
            ...
            if (userDoc.exists) {
              ...
              if (displayNameNullable != null &&
                  displayNameNullable.trim().isNotEmpty) {
                displayName = displayNameNullable;
              } else {
                // Fallback to email or username
                displayName =
                    userDocData['email']?.toString().split('@').first ??
                    userDocData['username'] as String? ??
                    'User ${participantId.substring(0, 8)}';
              }
              ...
              photoURL =
                  userDocData['photoURL'] as String? ??
          ...
          _logger.warning(
            'Error fetching participant data for $participantId: $e',
          );
    ...
    if (needsUpdate) {
      try {
        await _firestore
            ...
            .update({'participantData': participantData});
      } catch (e) {
  ...
  Stream<List<ChatMessage>> getConversationMessages(
