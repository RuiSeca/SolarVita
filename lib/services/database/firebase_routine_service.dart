...
class FirebaseRoutineService {
  static final FirebaseRoutineService _instance =
  ...
  Future<bool> syncUserRoutine(WorkoutRoutine routine) async {
    try {
      ...
      final routineData = {
        ...
        'dailyWorkouts': routine.weeklyPlan
            .map(
              (day) => {
                'dayName': day.dayName,
      ...
      if (e.toString().contains('permission-denied')) {
        _logger.severe(
          'Error syncing routine - Permission Denied: Please check Firestore security rules for user_routines collection',
        );
      } else {
  ...
  Future<List<ExerciseLog>> getRecentExerciseLogs(
    String userId, {
    int limit = 10,
  }) async {
    try {
  ...
  Future<Map<String, dynamic>?> getWeeklyProgress(
    ...
    try {
      ...
      if (progressSnapshot.docs.isEmpty) {
        ...
        return await _generateBasicProgress(
          userId,
          routineId,
          currentWeek,
          now.year,
  ...
  Future<Map<String, dynamic>> _generateBasicProgress(
    String userId,
    String routineId,
    int weekOfYear,
    int year,
    ...
    try {
      ...
      for (final logDoc in recentLogs.docs) {
        ...
        if (dayName != null &&
            exerciseId != null &&
            dailyProgress.containsKey(dayName)) {
          final completedIds = List<String>.from(
            dailyProgress[dayName]!['completedExerciseIds'],
          );
  ...
  Future<bool> copyRoutineFromUser(
    String sourceUserId,
    String routineId,
  ) async {
    try {
      ...
      if (copiedRoutineData['dailyWorkouts'] != null) {
        final dailyWorkouts = List<Map<String, dynamic>>.from(
          copiedRoutineData['dailyWorkouts'],
        );
