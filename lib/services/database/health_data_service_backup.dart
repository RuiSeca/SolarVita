...
class HealthDataService {
      ...
      if (hasPermissions == false) {
        ...
        if (granted || hasPermissionsAfterRequest == true) {
          ...
          return HealthPermissionStatus.granted(
            ...
            grantedPermissions: _healthDataTypes
                .map((type) => type.name)
                .toList(),
          );
        } else {
  ...
  Future<HealthData> _processHealthDataPoints(
      ...
      if (point.dateFrom.isAfter(last7Days)) {
        switch (point.type) {
          case HealthDataType.STEPS:
            final stepValue = (point.value as NumericHealthValue).numericValue
                .toInt();
            if (isToday) {
          ...
          case HealthDataType.HEART_RATE:
            final hrValue = (point.value as NumericHealthValue).numericValue
                .toDouble();
            heartRate += hrValue;
            heartRateCount++;
            break;
          ...
          case HealthDataType.SLEEP_REM:
            if (isToday ||
                pointDate.isAtSameMomentAs(
                  healthToday.subtract(const Duration(days: 1)),
                )) {
          ...
          case HealthDataType.WATER:
            final waterValue = (point.value as NumericHealthValue).numericValue
                .toDouble();
            if (isToday) {
          ...
          case HealthDataType.DISTANCE_DELTA:
            ...
            if (distance > 100 && duration.inMinutes > 0) {
              // Increase minimum to 100 meters
              ...
              if (pace >= 50) {
                // Only count if at least light walking pace
                final minutes = duration.inMinutes;
                // Apply intensity factor for more accurate active time
                final intensityFactor = pace >= 80
                    ? 1.0
                    : 0.7; // Reduce light activity impact
    ...
    int estimatedCalories = _estimateCaloriesFromActivity(
      todaySteps,
      totalTodayActiveMinutes,
    );

    // Combine health app water data with local tracking
    final combinedWaterIntake = await _getCombinedWaterIntake(
      todayWater > 0 ? todayWater : waterIntake,
    );
    ...
    final validatedSteps = _validateSteps(
      todaySteps,
    ); // Use actual today's steps only
    final validatedActiveMinutes = _validateActiveMinutes(
      totalTodayActiveMinutes,
    ); // Use actual today's active minutes only
    final validatedCalories = _validateCalories(
      estimatedCalories,
    ); // Use our estimated calories only
  ...
  Future<double> _getCombinedWaterIntake(double healthAppWater) async {
    try {
      ...
      return healthAppWater > localWaterIntake
          ? healthAppWater
          : localWaterIntake;
    } catch (e) {
  ...
  int _validateSteps(int steps) {
    ...
    if (steps < 0) return 0;
    if (steps > 100000) {
      return 100000; // Cap at 100k steps per day (extreme but possible)
    }
    return steps;
  }
  ...
  int _validateActiveMinutes(int activeMinutes) {
    ...
    if (activeMinutes > 1440) {
      return 1440; // Can't exceed 24 hours (1440 minutes)
    }
  ...
  int _validateCalories(int calories) {
    ...
    if (calories > 10000) {
      return 10000; // Cap at 10k calories (extreme athlete level)
    }
    return calories;
  }
            ...
            content: const SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '• Your phone\'s built-in step counter isn\'t connected\n',
                  ),
                  ...
                  Text(
                    '3. Connect apps like Samsung Health, Fitbit, or other health apps\n',
                  ),
