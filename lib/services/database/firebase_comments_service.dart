...
class FirebaseCommentsService {
  static final FirebaseCommentsService _instance =
  ...
  Stream<List<PostComment>> getCommentReplies(
    String commentId, {
    int limit = 20,
  }) {
    return _firestore
  ...
  Stream<List<CommentThread>> getCommentThreads(
    String postId, {
    int limit = 20,
  }) {
  ...
  Future<void> reactToComment(String commentId, ReactionType reaction) async {
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      final updatedReactions = Map<String, ReactionType>.from(
        comment.reactions,
      );
      ...
      transaction.update(commentRef, {
        'reactions': _reactionsToFirestore(updatedReactions),
      });
    });
  }
  ...
  Future<String> _uploadCommentMedia(String commentId, File file) async {
    ...
    final ref = _storage.ref().child(
      'comments/$commentId/$fileName.$extension',
    );
  ...
  Future<void> _incrementCommentReplyCount(
    String parentCommentId,
    String replyId,
  ) async {
  ...
  Future<void> _decrementCommentReplyCount(
    String parentCommentId,
    String replyId,
  ) async {
    final commentRef = _firestore
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      if (doc.exists) {
        ...
        if (updatedChildIds.contains(replyId)) {
          ...
          final newCount = (comment.replyCount - 1)
  ...
  Map<String, String> _reactionsToFirestore(
