...
class TribeService {
  ...
  Future<String> createTribe({
    ...
    final inviteCode = visibility == TribeVisibility.private
        ? _generateInviteCode()
        : null;
    ...
    // Create the tribe
    final tribeRef = await _firestore
        .collection('tribes')
        .add(tribe.toFirestore());
    ...
    return query.snapshots().map((snapshot) {
      ...
      if (searchQuery != null && searchQuery.isNotEmpty) {
        return tribes
            .where(
              (tribe) =>
                  tribe.name.toLowerCase().contains(
  ...
  Stream<List<TribePost>> getTribePosts(String tribeId, {int limit = 20}) {
    return _firestore
        ...
        .map(
          (snapshot) =>
              snapshot.docs.map((doc) => TribePost.fromFirestore(doc)).toList(),
        );
  }
  ...
  static List<TribeCategory> getAllCategories() {
    return TribeCategory.values
        .where((cat) => cat != TribeCategory.custom)
        .toList();
  }
  ...
  Stream<List<Tribe>> getUserTribes() {
    ...
    return _firestore
        ...
        .asyncMap((snapshot) async {
          ...
          final tribeIds = snapshot.docs
              .map((doc) => doc.data()['tribeId'] as String)
              .toList();
  ...
  Stream<List<TribePost>> getAllTribesActivityFeed({int limit = 20}) {
    ...
    return getUserTribes()
        .asyncMap((tribes) async {
          if (tribes.isEmpty) {
            return <TribePost>[];
          }
