...
class SocialService {
  ...
  static void setSupporterCountChangeCallback(
    SupporterCountChangeCallback callback,
  ) {
  ...
  Future<void> sendSupporterRequest(
    String receiverId, {
    String? message,
  }) async {
    ...
    final existingSupporterRequest = await _checkExistingSupporterRequest(
      receiverId,
    );
    ...
    try {
      ...
      debugPrint(
        '✅ Support request notification sent (will show only if app in background)',
      );
    } catch (e) {
  ...
  Future<void> acceptSupporterRequest(String supporterRequestId) async {
    final supporterRequestDoc = await _firestore
        .collection('supporterRequests')
        .doc(supporterRequestId)
        .get();
    ...
    try {
      ...
      if (existingSupport == null) {
        await supportUser(
          requesterId,
        ); // This now creates BOTH relationships and updates BOTH counts
      }
        ...
        .asyncMap((snapshot) async {
          ...
          for (final doc in snapshot.docs) {
            try {
              ...
              final supporterDoc = await _firestore
                  .collection('users')
                  .doc(supporterId)
                  .get();
              if (supporterDoc.exists) {
                ...
                String? displayNameNullable =
                    userData['displayName'] as String?;
                String displayName;
                if (displayNameNullable == null ||
                    displayNameNullable.trim().isEmpty) {
                  displayName =
        ...
        .asyncMap((snapshot) async {
          ...
          for (final doc in snapshot.docs) {
            try {
              ...
              final supportedDoc = await _firestore
                  .collection('users')
                  .doc(supportedId)
                  .get();
  ...
  Future<void> supportUser(String userId) async {
    ...
    try {
      ...
      final supporterDoc = await _firestore
          ...
          .doc(currentUserId)
          .get();
      final supportingDoc = await _firestore
          .collection('users')
          .doc(userId)
          .get();
      ...
      final reverseSupport = await _checkSupportFromUser(
        userId,
        currentUserId!,
      );
  ...
  Future<Support?> _checkSupportFromUser(
    String supporterId,
    String supportedId,
  ) async {
    try {
  ...
  Future<void> _createSupportRelationship(
    String supporterId,
    String supportedId,
  ) async {
    try {
  ...
  Future<void> updateChallengeProgress(String challengeId, int progress) async {
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      final leaderboard = Map<String, int>.from(
  ...
  Future<List<Supporter>> searchUsers(String query) async {
    ...
    results.sort((a, b) {
      ...
      // Exact matches first
      if (aUsername == normalizedQuery || aDisplayName == normalizedQuery) {
        return -1;
      }
      if (bUsername == normalizedQuery || bDisplayName == normalizedQuery) {
        return 1;
      }

      // Starts with query
      if (aUsername.startsWith(normalizedQuery) ||
          aDisplayName.startsWith(normalizedQuery)) {
        return -1;
      }
      if (bUsername.startsWith(normalizedQuery) ||
          bDisplayName.startsWith(normalizedQuery)) {
        return 1;
      }
  ...
  Future<void> logEcoActivity({
    ...
    await createActivity(
      ...
      title: action,
      description: impact,
      metadata: {'action': action, 'impact': impact},
    );
  }

  // Helper method to check existing supporterRequest
  Future<SupporterRequest?> _checkExistingSupporterRequest(
    String otherUserId,
  ) async {
    if (currentUserId == null) {
      return null;
    ...
    if (snapshot1.docs.isNotEmpty) {
      final supporterRequest = SupporterRequest.fromFirestore(
  ...
  Future<SupporterRequestStatus?> getSupporterRequestStatus(
    String otherUserId,
  ) async {
  ...
  Future<bool> hasActiveSupporterRequest(String otherUserId) async {
    ...
    final theySupportMe = await _checkSupportFromUser(
      otherUserId,
      currentUserId!,
    );
  ...
  Future<void> migrateSupporterRequestsToMutualSupporting() async {
    ...
    try {
      ...
      final allSupporterRequests = [
        ...supporterRequestsAsRequester.docs,
        ...supporterRequestsAsReceiver.docs,
      ];
      ...
      for (final supporterRequestDoc in allSupporterRequests) {
        ...
        final requesterSupportsReceiver = await _checkSupportFromUser(
          requesterId,
          receiverId,
        );
        ...
        final receiverSupportsRequester = await _checkSupportFromUser(
          receiverId,
          requesterId,
        );
    ...
    try {
      ...
      final result = {
        ...
        'supporterRequests': {
          ...
          'total':
              supporterRequestsAsRequester.docs.length +
              supporterRequestsAsReceiver.docs.length,
        },
        'supports': {
        ...
        'details': {
          'supporterRequests': [
            ...supporterRequestsAsRequester.docs.map(
              (doc) => {
                'id': doc.id,
                'type': 'asRequester',
                'data': doc.data(),
            ...
            ...supporterRequestsAsReceiver.docs.map(
              (doc) => {'id': doc.id, 'type': 'asReceiver', 'data': doc.data()},
            ),
          ],
          'supporting': supporting.docs
              .map((doc) => {'id': doc.id, 'data': doc.data()})
              .toList(),
          'supporters': supporters.docs
              .map((doc) => {'id': doc.id, 'data': doc.data()})
              .toList(),
        },
      };
