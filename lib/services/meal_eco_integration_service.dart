...
import 'eco_service.dart';

/// Service to automatically generate eco activities from meal logging
class MealEcoIntegrationService {
  static final MealEcoIntegrationService _instance = MealEcoIntegrationService._internal();
  ...
  Future<String?> onMealLogged({
    ...
    try {
      ...
      if (carbonSaved > 0.1) { // Minimum threshold of 100g CO₂
        ...
        debugPrint('✅ Eco activity created for meal: $mealName (${carbonSaved.toStringAsFixed(2)}kg CO₂ saved)');
        return activityId;
      } else {
        debugPrint('ℹ️ No eco activity created for $mealCategory meal (insufficient carbon savings)');
        return null;
      }
    } catch (e) {
  ...
  String _categorizeFoodAnalysis(FoodAnalysis foodAnalysis) {
    ...
    final ingredients = foodAnalysis.ingredients.map((i) => i.toLowerCase()).toList();
    
    // Check for meat types
    if (_containsAny(foodName, ingredients, ['beef', 'steak', 'hamburger', 'ground beef'])) {
      return 'beef';
    }
    if (_containsAny(foodName, ingredients, ['chicken', 'poultry', 'wing', 'breast'])) {
      return 'chicken';
    }
    if (_containsAny(foodName, ingredients, ['pork', 'bacon', 'ham', 'sausage'])) {
      return 'pork';
    }
    if (_containsAny(foodName, ingredients, ['lamb', 'mutton'])) {
      return 'lamb';
    }
    if (_containsAny(foodName, ingredients, ['fish', 'salmon', 'tuna', 'shrimp', 'crab', 'lobster'])) {
      return 'seafood';
    }
    ...
    if (_containsAny(foodName, ingredients, ['pasta', 'spaghetti', 'noodle', 'macaroni'])) {
      return 'pasta';
    }
    if (_containsAny(foodName, ingredients, ['dessert', 'cake', 'cookie', 'ice cream', 'chocolate'])) {
      return 'dessert';
    }
    if (_containsAny(foodName, ingredients, ['breakfast', 'cereal', 'pancake', 'waffle', 'toast'])) {
      return 'breakfast';
    }
  ...
  bool _containsAny(String foodName, List<String> ingredients, List<String> terms) {
    for (String term in terms) {
      if (foodName.contains(term) || ingredients.any((ingredient) => ingredient.contains(term))) {
        return true;
      }
    }
    return false;
  ...
  bool _isVeganMeal(String foodName, List<String> ingredients) {
    final veganIndicators = ['vegan', 'plant based', 'tofu', 'tempeh', 'seitan'];
    final nonVeganIndicators = ['meat', 'chicken', 'beef', 'pork', 'fish', 'dairy', 'cheese', 'milk', 'egg', 'butter'];
    ...
    final plantIndicators = ['vegetable', 'fruit', 'grain', 'bean', 'lentil', 'quinoa', 'rice', 'salad'];
  ...
  bool _isVegetarianMeal(String foodName, List<String> ingredients) {
    ...
    final meatIndicators = ['meat', 'chicken', 'beef', 'pork', 'fish', 'seafood'];
