...
class DynamicDurationService {
  static final DynamicDurationService _instance = DynamicDurationService._internal();
  ...
  Future<double> calculateDynamicDuration({
    ...
    try {
      ...
      _log.info('Found ${logs.length} logs for $exerciseName (ID: $exerciseId)');
      
      if (logs.isEmpty) {
        // No historical data, use static duration with planned multiplier
        final result = _calculateStaticWithMultiplier(staticDuration, plannedSets, plannedReps);
        _log.info('No logs for $exerciseName, using static multiplier: ${result.toStringAsFixed(1)} min (static: $staticDuration, sets: $plannedSets)');
        return result;
      }
      ...
      final totalPlannedSets = plannedSets ?? _getAverageSetsFromHistory(recentLogs);
      ...
      if (plannedWeight != null && plannedWeight > 0) {
        ...
        if (avgHistoricalWeight > 0) {
          ...
          dynamicDuration *= (1 + (weightMultiplier - 1) * 0.3); // Max 30% increase
        }
      }
      ...
      _log.info('Dynamic duration for $exerciseName: ${clampedDuration.toStringAsFixed(1)} min (static: $staticDuration, before clamp: ${dynamicDuration.toStringAsFixed(1)})');
      ...
      _log.warning('Error calculating dynamic duration for $exerciseName: $e, falling back to ${safeFallback.toStringAsFixed(1)} min');
      return safeFallback;
    }
  }

  /// Calculate total dynamic duration for a day's workout
  Future<DynamicWorkoutDuration> calculateDayDynamicDuration(DailyWorkout day) async {
    ...
    for (final exercise in day.exercises) {
      ...
      _log.info('Processing ${exercise.title}: ${exercise.steps.length} steps (instructions), using $plannedSets sets');
      ...
      exerciseComparisons.add(ExerciseDurationComparison(
        exerciseName: exercise.title,
  ...
  double _calculateAverageTimePerSet(List<ExerciseLog> logs) {
    ...
    return avgTimePerSet.clamp(0.5, 5.0); // Between 30 seconds and 5 minutes per set
  }
  ...
  int _getAverageSetsFromHistory(List<ExerciseLog> logs) {
    ...
    final avgSets = logs.map((log) => log.sets.length)
  ...
  double _getExerciseTypeMultiplier(String exerciseName) {
    ...
    // Cardio exercises
    if (name.contains('run') || name.contains('cycle') || name.contains('cardio')) {
  ...
  double _calculateStaticWithMultiplier(String staticDuration, int? plannedSets, int? plannedReps) {
    final baseDuration = _parseStaticDuration(staticDuration);
    _log.info('Parsed static duration "$staticDuration" as ${baseDuration.toStringAsFixed(1)} min');
    ...
    if (plannedSets != null && plannedSets > 0) {
      ...
      _log.info('Applied sets multiplier: ${baseDuration.toStringAsFixed(1)} * $clampedSets = ${result.toStringAsFixed(1)} min');
      return result;
    }
    
    _log.info('No sets multiplier applied, returning base duration: ${baseDuration.toStringAsFixed(1)} min');
