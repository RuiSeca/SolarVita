...
class ExerciseTrackingService {
  ...
  Future<bool> updateLog(ExerciseLog updatedLog) async {
    try {
      ...
      List<ExerciseLog> logObjects = logs
          .map((log) => ExerciseLog.fromJson(jsonDecode(log)))
          .toList();
      ...
      if (index != -1) {
        ...
        List<String> updatedLogs = logObjects
  ...
  Future<bool> _checkForPersonalRecords(ExerciseLog log) async {
    try {
      ...
      PersonalRecord? existingMaxWeightRecord = records
      ...
      PersonalRecord? existingVolumeRecord = records
          .where((r) => r.recordType == 'Volume')
          .firstOrNull;
  ...
  Future<bool> savePersonalRecord(PersonalRecord record) async {
    try {
      ...
      List<PersonalRecord> recordObjects = records
          .map((r) => PersonalRecord.fromJson(jsonDecode(r)))
          .toList();

      // Remove any existing record of the same type for this exercise
      recordObjects.removeWhere(
        (r) =>
      ...
      List<String> updatedRecords = recordObjects
