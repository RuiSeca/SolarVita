...
import '../database/routine_service.dart';
import '../database/firebase_routine_service.dart';

class ExerciseRoutineSyncService {
  static final ExerciseRoutineSyncService _instance =
  ...
  Future<bool> logExerciseToRoutine({
    ...
    try {
      ...
      _log.info(
        'Exercise log saved: $saved for ${log.exerciseName} (ID: ${log.exerciseId})',
      );
  ...
  Future<WeeklyProgress?> getWeeklyProgress(
    ...
    try {
      ...
      for (final data in progressData) {
        ...
        if (progress.routineId == routineId &&
            progress.weekOfYear == targetWeek) {
  ...
  Future<Map<String, dynamic>> getAutoFillData(
    ...
    try {
      ...
      final currentWeekLogs = logs
          .where((log) => log.weekNumber == currentWeek)
          .toList();
  ...
  Future<bool> isExerciseCompletedForDay(
    String routineId,
    String dayName,
    String exerciseId,
  ) async {
  ...
  Future<List<ExerciseLog>> getWeeklyLogsByRoutineAndDay(
    String routineId,
    String dayName,
  ) async {
    try {
  ...
  Future<Map<String, dynamic>> getRoutineCompletionStats(
    String routineId, {
    int? weekOfYear,
  }) async {
    try {
      ...
      final stats = {
        ...
        'dailyBreakdown': progress.dailyProgress.map(
          (day, progress) => MapEntry(day, {
  ...
  Future<int> _calculateCurrentStreak(String routineId) async {
    try {
      final allLogs = await _exerciseService.getAllLogs();
      final routineLogs =
          allLogs.where((log) => log.routineId == routineId).toList()..sort(
  ...
  Future<WeeklyProgress> _createWeeklyProgress(
    String routineId,
    int weekOfYear,
  ) async {
    try {
    ...
    try {
      ...
      if (!dayProgress.completedExerciseIds.contains(exerciseId)) {
        final updatedDayProgress = dayProgress.copyWith(
          completedExerciseIds: [
            ...dayProgress.completedExerciseIds,
            exerciseId,
          ],
          lastUpdated: DateTime.now(),
        );

        final updatedDailyProgress = Map<String, DayProgress>.from(
  ...
  Future<bool> _checkAndUpdatePersonalRecords(ExerciseLog log) async {
    try {
      ...
      final existingRecords = await getPersonalRecordsForExercise(
        log.exerciseId,
      );
      ...
      if (log.maxDuration != null) {
        recordChecks.add({
          'type': 'Max Duration',
          'value': log.maxDuration!.inSeconds.toDouble(),
        });
      }
      ...
      for (final check in recordChecks) {
        ...
        if (existingRecord == null ||
            (check['value'] as double) > existingRecord.value) {
  ...
  Map<String, dynamic> _generateSuggestions(
    ExerciseLog lastLog,
    ...
    if (lastLog.sets.isNotEmpty) {
      final avgWeight =
          lastLog.sets.map((s) => s.weight).reduce((a, b) => a + b) /
          lastLog.sets.length;
      suggestions['recommendedWeight'] = (avgWeight * 1.025)
          .round(); // 2.5% increase
    }
    ...
    if (lastLog.sets.isNotEmpty) {
      final avgReps =
          lastLog.sets.map((s) => s.reps).reduce((a, b) => a + b) /
          lastLog.sets.length;
      suggestions['recommendedReps'] = (avgReps + 1).round();
    }

    // Personal record context
    final maxWeightRecord = records
        .where((r) => r.recordType == 'Max Weight')
        .firstOrNull;
  ...
  Future<void> _saveExerciseHistory(ExerciseLog log) async {
    try {
      ...
      _log.info(
        'Exercise history saved for Firestore migration: ${log.exerciseName}',
      );
    } catch (e) {
  ...
  Future<void> markHistoryAsSynced(List<String> historyIds) async {
    try {
      ...
      await prefs.setStringList(_exerciseHistoryKey, historyData);
      _log.info(
        'Marked ${historyIds.length} history entries as synced to Firestore',
      );
    } catch (e) {
  ...
  void _syncInBackground(String routineId, int targetWeek, String cacheKey) {
    ...
    Future.microtask(() async {
      try {
        await syncPlannedExercisesWithRoutine(
          routineId,
  ...
  Future<void> syncPlannedExercisesWithRoutine(
    String routineId, {
    int? weekOfYear,
  }) async {
    try {
      ...
      for (final data in progressData) {
        ...
        if (candidate.routineId == routineId &&
            candidate.weekOfYear == targetWeek) {
          progress = candidate;
          break;
        }
      }
      ...
      final currentTotalExercises = routine.weeklyPlan.fold(
        0,
        (sum, day) => sum + day.exercises.length,
      );
      final cachedTotalExercises = progress.dailyProgress.values.fold(
        0,
        (sum, day) => sum + day.plannedExercises,
      );
