...
import '../models/post_comment.dart' as post_comment_model;
...
class CachedItem<T> {
  ...
  static CachedItem<T> fromJson<T>(Map<String, dynamic> json, T Function(Map<String, dynamic>) deserializer) {
    return CachedItem<T>(
      key: json['key'],
...
class OfflineCacheService {
  ...
  Future<List<T>?> getCachedList<T>({
    ...
    final cachedList = await getCachedData<List<dynamic>>(
      type: type,
      key: key,
    );
  ...
  Future<void> cacheSocialPosts(List<SocialPost> posts, {String? feedType}) async {
    ...
    await cacheList<SocialPost>(
      ...
      key: key,
      items: posts,
      expiry: const Duration(minutes: 30), // Posts expire quickly for fresh content
    );
  }
  ...
  Future<List<SocialPost>?> getCachedSocialPosts({String? feedType}) async {
    ...
    return await getCachedList<SocialPost>(
      type: CacheType.socialPosts,
      key: key,
      itemDeserializer: (json) => _deserializeFromCache<SocialPost>(json, 'SocialPost'),
    );
  }
  ...
  Future<List<ChatConversation>?> getCachedChatConversations() async {
    return await getCachedList<ChatConversation>(
      type: CacheType.conversations,
      key: 'user_conversations',
      itemDeserializer: (json) => _deserializeFromCache<ChatConversation>(json, 'ChatConversation'),
    );
  }
  ...
  Future<List<ChatMessage>?> getCachedChatMessages(String conversationId) async {
    return await getCachedList<ChatMessage>(
      type: CacheType.chatMessages,
      key: conversationId,
      itemDeserializer: (json) => _deserializeFromCache<ChatMessage>(json, 'ChatMessage'),
    );
  }
  ...
  Future<UserProfile?> getCachedUserProfile(String userId) async {
    return await getCachedData<UserProfile>(
      type: CacheType.userProfiles,
      key: userId,
      deserializer: (json) => _deserializeFromCache<UserProfile>(json, 'UserProfile'),
    );
  }
  ...
  Future<String?> cacheMediaFile(String url, {Duration? expiry}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Store metadata
        await _prefs.setString('${cacheKey}_cached_at', DateTime.now().toIso8601String());
        if (expiry != null) {
          await _prefs.setString('${cacheKey}_expires_at', DateTime.now().add(expiry).toIso8601String());
        }
  ...
  Future<void> _cleanExpiredMediaFiles() async {
    final keys = _prefs.getKeys().where((key) => key.endsWith('_expires_at')).toList();
  ...
  Future<CachedItem<T>?> _loadFromDisk<T>(String key, T Function(Map<String, dynamic>)? deserializer) async {
    try {
      final file = File('$_cacheDirectory/$key.cache');
  ...
  Future<void> clearAllCache() async {
    ...
    final keys = _prefs.getKeys().where((key) => 
        ...
        key.contains('_expires_at')).toList();
...
extension CacheExtensions on FirebaseFirestore {
  ...
  Future<void> enableOfflineSupport() async {
    ...
    try {
      ...
      logger.info('Firestore persistence should be configured during initialization');
    } catch (e) {
