...
class NutritionixService {
  ...
  Future<MultiIngredientAnalysis> analyzeFoodImageAdvanced(File image) async {
    try {
      ...
      final List<String> identifiedFoods = await _recognitionService
          .identifyFoodInImage(image);
      ...
      for (String foodName in identifiedFoods) {
        try {
          final FoodAnalysis? nutritionData = await getNaturalNutrients(
            foodName,
          );

          if (nutritionData != null) {
            ingredientAnalyses.add(
              IngredientAnalysis(
                name: foodName,
      ...
      final primaryFood = _determinePrimaryFood(
        identifiedFoods,
        ingredientAnalyses,
      );
      ...
      return MultiIngredientAnalysis(
        ...
        detectionAccuracy: _calculateOverallAccuracy(
          ingredientAnalyses,
          failedIngredients,
        ),
  ...
  PortionEstimate _estimateStandardPortion(String foodName) {
    ...
    final Map<String, PortionEstimate> standardPortions = {
      // Grains & Starches
      'rice': PortionEstimate(
        amount: 150,
        unit: 'g',
        description: '1 cup cooked',
      ),
      'pasta': PortionEstimate(
        amount: 125,
        unit: 'g',
        description: '1 cup cooked',
      ),
      'bread': PortionEstimate(amount: 30, unit: 'g', description: '1 slice'),
      'quinoa': PortionEstimate(
        amount: 185,
        unit: 'g',
        description: '1 cup cooked',
      ),
      'oats': PortionEstimate(
        amount: 40,
        unit: 'g',
      ...
      'chicken': PortionEstimate(
        amount: 100,
        unit: 'g',
        description: '1 palm-sized piece',
      ),
      'beef': PortionEstimate(
        amount: 85,
        unit: 'g',
        description: '3 oz serving',
      ),
      'fish': PortionEstimate(amount: 100, unit: 'g', description: '1 fillet'),
      'salmon': PortionEstimate(
        amount: 100,
        unit: 'g',
        description: '1 fillet',
      ),
      'tuna': PortionEstimate(
        amount: 85,
        unit: 'g',
        description: '3 oz serving',
      ),
      ...
      // Vegetables
      'broccoli': PortionEstimate(
        amount: 80,
        unit: 'g',
        description: '1/2 cup',
      ),
      'spinach': PortionEstimate(
        amount: 30,
        unit: 'g',
      ...
      // Nuts & Seeds
      'nuts': PortionEstimate(
        amount: 30,
        unit: 'g',
        description: '1 oz handful',
      ),
      'almonds': PortionEstimate(
        amount: 30,
        unit: 'g',
        description: '23 almonds',
      ),
    ...
    if (_isVegetable(foodLower)) {
      ...
      return PortionEstimate(
        amount: 100,
        unit: 'g',
        description: '1 palm-sized serving',
      );
  ...
  double _calculateConfidence(String foodName) {
    ...
    if (highConfidenceFoods.any(
      (food) => foodLower.contains(food.toString()),
    )) {
      return 0.9;
    } else if (mediumConfidenceFoods.any(
      (food) => foodLower.contains(food.toString()),
    )) {
      return 0.7;
    } else if (lowConfidenceFoods.any(
      (food) => foodLower.contains(food.toString()),
    )) {
      return 0.3;
    }
  ...
  CombinedNutrition _calculateCombinedNutrition(
    ...
    for (var ingredient in ingredients) {
      ...
      final portionMultiplier =
          ingredient.estimatedPortion.amount /
  ...
  String _determinePrimaryFood(
    ...
    List<IngredientAnalysis> successful,
  ) {
  ...
  double _calculateOverallAccuracy(
    ...
    double weightedSuccess = successful.fold(
      0.0,
  ...
  Future<FoodAnalysis?> getNaturalNutrients(String query) async {
    try {
      final uri = Uri.parse(
        '${NutritionixApiConfig.baseUrl}/natural/nutrients',
      );

      final requestBody = {'query': query, 'timezone': 'US/Eastern'};
  ...
  FoodAnalysis _parseInstantSearchFood(
    Map<String, dynamic> foodData, {
    required bool isCommon,
  }) {
    try {
      final String foodId =
          foodData['nix_item_id'] ??
          foodData['food_name'] ??
  ...
  FoodAnalysis _parseNaturalNutrientsFood(Map<String, dynamic> foodData) {
    try {
      ...
      final int healthRating = _calculateHealthRating(
        calories,
        protein,
        carbs,
        fat,
