...
class MealDBService {
  ...
  static const int _minRequestInterval =
      50; // Reduced to 50ms between requests for faster initial load
      ...
      if (response.statusCode == 429) {
        if (retryCount < maxRetries) {
          final backoffDelay = Duration(
            milliseconds: 1000 * (1 << retryCount),
  ...
  Future<List<Map<String, dynamic>>> getMealsByCategory(String category) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['meals'] != null) {
        ...
        List<Map<String, dynamic>> basicMeals = mealList
            .map(
              (meal) => {
                'idMeal': meal['idMeal'] ?? '',
  ...
  Map<String, dynamic> _formatMealData(Map<String, dynamic> meal) {
    ...
    final formattedMeal = {
      ...
      'nutritionFacts': {
        ...
        'caloriesPerServing':
            '${(approximateCalories / _estimateServings(ingredients)).round()}',
        'protein': '${approximateCalories ~/ 20}g', // Rough estimation
        'proteinPerServing':
            '${(approximateCalories ~/ 20 / _estimateServings(ingredients)).round()}g',
        'carbs': '${approximateCalories ~/ 15}g', // Rough estimation
        'carbsPerServing':
            '${(approximateCalories ~/ 15 / _estimateServings(ingredients)).round()}g',
        'fat': '${approximateCalories ~/ 30}g', // Rough estimation
        'fatPerServing':
            '${(approximateCalories ~/ 30 / _estimateServings(ingredients)).round()}g',
      },
