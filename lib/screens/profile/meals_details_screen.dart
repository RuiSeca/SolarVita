...
class _MealsDetailsScreenState extends ConsumerState<MealsDetailsScreen> {
  ...
  Widget _buildHeroSection(BuildContext context, WidgetRef ref, EcoMetrics metrics, MealState mealsState) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ...todaysMeals.map((meal) => _buildDetailedMealCard(context, ref, meal)),
        ],
      ),
    );
  }
  ...
  Widget _buildDetailedMealCard(BuildContext context, WidgetRef ref, Map<String, dynamic> meal) {
  ...
  Widget _buildSustainabilityAnalysis(BuildContext context, WidgetRef ref, MealState mealsState, EcoMetrics metrics) {
    final todaysMeals = mealsState.meals?.take(10).toList() ?? [];
  ...
  Widget _buildAnalysisCard(BuildContext context, String emoji, String value, String label, Color color) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildMealCategoriesSection(BuildContext context, WidgetRef ref, MealState mealsState) {
    final todaysMeals = mealsState.meals?.take(10).toList() ?? [];
  ...
  Widget _buildWeeklyTrendsSection(BuildContext context, WidgetRef ref, EcoMetrics metrics) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: AppTheme.textFieldBackground(context),
  ...
  Widget _buildTrendCard(BuildContext context, String title, String value, IconData icon, Color color) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: color.withValues(alpha: 0.1),
  ...
  Widget _buildNutritionSustainabilitySection(BuildContext context, WidgetRef ref, MealState mealsState) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        gradient: LinearGradient(
      ...
      child: Column(
        ...
        children: [
          ...
          // Balance indicators
          _buildBalanceIndicator('Protein Sources', 0.7, Colors.red, 'Consider more plant-based proteins'),
          const SizedBox(height: 12),
          _buildBalanceIndicator('Vegetable Intake', 0.9, Colors.green, 'Excellent plant diversity!'),
          const SizedBox(height: 12),
          _buildBalanceIndicator('Processed Foods', 0.3, Colors.orange, 'Great job minimizing processed foods'),
          const SizedBox(height: 12),
          _buildBalanceIndicator('Local/Seasonal', 0.6, Colors.blue, 'Try incorporating more seasonal ingredients'),
        ],
      ),
    );
  }

  Widget _buildBalanceIndicator(String label, double value, Color color, String tip) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
  ...
  Widget _buildMealAchievementsSection(BuildContext context, WidgetRef ref, EcoMetrics metrics, MealState mealsState) {
    ...
    final tips = [
      {
        ...
        'description': 'Plant-based meals typically have 50-90% lower carbon footprint than meat-based meals.',
        'color': Colors.green,
      },
      {
        'icon': Icons.location_on,
        'title': 'Eat Local & Seasonal',
        'description': 'Local produce reduces transportation emissions and supports your community.',
        'color': Colors.blue,
      },
      {
        'icon': Icons.recycling,
        'title': 'Minimize Food Waste',
        'description': 'Plan portions carefully and use leftovers creatively to reduce waste.',
        'color': Colors.orange,
      },
      {
        'icon': Icons.water_drop,
        'title': 'Consider Water Footprint',
        'description': 'Some foods require significantly more water to produce than others.',
        'color': Colors.cyan,
      },
    ];
  ...
  Widget _buildActionButtonsSection(BuildContext context, WidgetRef ref, MealState mealsState) {
    final todaysMeals = mealsState.meals?.take(10).toList() ?? [];
  ...
  List<Map<String, dynamic>> _calculateMealAchievements(EcoMetrics metrics, MealState mealsState) {
    ...
    if (metrics.mealCarbonSaved >= 1.0) {
      achievements.add({
        'emoji': '🌱',
        'title': 'Eco Eater',
        'description': 'Saved ${metrics.mealCarbonSaved.toStringAsFixed(1)}kg CO₂ through sustainable meal choices!',
      });
    }
    ...
    if (metrics.currentStreak >= 7) {
      achievements.add({
        ...
        'description': 'Maintained sustainable eating for ${metrics.currentStreak} days!',
      });
    }
    ...
    if (todaysMeals.length >= 5) {
      achievements.add({
        ...
        'description': 'Logged ${todaysMeals.length} meals today - great tracking!',
      });
    }
